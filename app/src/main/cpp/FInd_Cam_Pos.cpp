...
vector<int>& ball_color_ref, Mat& in_rvec, Mat& in_tvec){
    ...
    Mat t_i;
    temp_img.copyTo(t_i);
    Ball_and_Sol_templete.copyTo(temp);
    int j = dist[k].second;
    ...
    for(int i=0; i<4; i++){
        ...
        if(i==0 || i ==1){
            int x = (int)match_list1[j][i].x;
            int y = (int)match_list1[j][i].y;
            ...
            putText(temp, str, Point(x, y), 1,10, Scalar(255,255,0), 2,8);
        }        

        if(i==2 || i==3)
            putText(temp, str, Point((int)match_list1[j][i].x, (int)match_list1[j][i].y), 1,10, Scalar(255,255,0), 2,8);
            
        putText( t_i, str, Point((int)match_list2[j][i].x, (int)match_list2[j][i].y), 1,4, Scalar(255,255,0), 2,8);
    ...
    Draw_3D_Templete_on_Img(t_i);

    // 안쪽테두리 표시
    vector<Point2d> object_img_pt;
    projectPoints(world_table_inside_corners_d, in_rvec, in_tvec, INTRINSIC, distCoeffs, object_img_pt);

    for(int i=0; i<4; i++){
        line(t_i, object_img_pt[i%4], object_img_pt[(i+1)%4], Scalar(0,255,255), 2);
    }
 

    resize(temp, temp,Size(500, 1000));
    //imshow("result", temp);
    //imshow("img",  t_i);
    //waitKey();
    }
